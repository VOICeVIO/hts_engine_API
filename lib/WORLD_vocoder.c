...
#pragma comment (lib, "hts_engine_API\\World\\World.lib")
#pragma comment (lib, "hts_engine_API\\SPTK\\lib\\SPTK.lib")
...
/* FIXME: */
static void SPTK_mgc2sp_bap(WORLD_Vocoder * v, double **mcep, double alpha, double gamma, size_t mcep_dim)
{
	size_t i, j;
	double *sp_buff;
	double *phase_buff;
	...
	for (i = 0; i < v->total_frame; i++) {
		mgc2sp(mcep[i], mcep_dim - 1, alpha, 0 /* gamma */, sp_buff, phase_buff, v->fft_size);
		for (j = 0; j < v->aperiodicity_size; j++)
			v->aperiodicity_buff[i][j] = exp(sp_buff[j] * 2);
	}
...
/* FIXME: Badly modified WORLD based waveform synthesis */
void WORLD_Vocoder_synthesize_v2(WORLD_Vocoder * v, double **lf0, double **mcep, size_t mcep_dim, double alpha, double gamma, double **band_aperiodicity, size_t bap_dim, double *rawdata, HTS_Audio * audio)
{
	size_t i, wav_length;
	...
	{
		//v->f0_buff[i] = lf0[i][0] > 0 ? exp(lf0[i][0]) : 0.0;
		v->f0_buff[i] = lf0[i][0] > -1e10 ? exp(lf0[i][0]) : 0.0; //MODIFIED
		//printf("%lf\t%lf\t\r\n", lf0[i][0], v->f0_buff[i]);
	}
	...
	/* band aperiodicity -> WORLD's aperiodicity */
	/* MODIFIED: BAP->AP from https://github.com/npuichigo/ttsflow/blob/2b148b5addecbf67e3f2784f06f8f9d2bf80bffc/ttsflow/vocoder/world_vocoder/world_vocoder.cc#L149 */
	//SPTK_mgc2sp_bap(v, band_aperiodicity, alpha, gamma, bap_dim);
	//V2: 5 band -> any band
	/* 5-band aperiodicity -> WORLD's aperiodicity */
	...
	for (i = 0; i < v->total_frame; i++) {
		...
		{
			//printf("%lf\r\n", band_aperiodicity[i][k]);
			const int step = 512 / bap_dim;
			for (size_t j = k * step; j < (k + 1) * step; j++)
			{
				if (j > 512)
				{
					continue;
				}
				v->aperiodicity_buff[i][j] = band_aperiodicity[i][k];
			}
		}
	}
	//v->aperiodicity_buff = band_aperiodicity;
